...
def getImage(root='demo'):
    ...
    return D, RD
'''
def getDepthImage(rootdir,name_path):
    #D = cv2.imread(os.path.join(root, '0.png'), cv2.COLOR_BGR2GRAY)/10000
    D = cv2.imread(os.path.join(rootdir, name_path), cv2.COLOR_BGR2GRAY)/10000
    print(os.path.join(rootdir, name_path))
    return D

def getRawDepthImage(rootdir,name_path):
    D = cv2.imread(os.path.join(rootdir, name_path), cv2.COLOR_BGR2GRAY)/10000
    return D
...
if __name__ == "__main__":
    ...
    save_path_cohha='./nyu_hha_complete/'
    for i in range(1449):
        print(str("%04d" % (i+1)))
        ...
        cv2.imwrite(save_path_hha+'image_'+str("%04d" % (i+1))+'.png', hha)
        cv2.imwrite(save_path_cohha+'image_'+str("%04d" % (i+1))+'.png', hha_complete)